...
public class ScrollView extends FrameLayout {
	...
	protected float getBottomFadingEdgeStrength() {
		...
		final int length = getVerticalFadingEdgeLength();
		final int bottomEdge = getHeight() - paddingBottom;
	...
	/*private void initDifferentialFlingHelperIfNotExists() {
		if (mDifferentialMotionFlingHelper == null) {
			mDifferentialMotionFlingHelper =
				...
				getContext(), new DifferentialFlingTarget());
		}
	}*/
	...
	public boolean performAccessibilityActionInternal(int action, Bundle arguments) {
		...
		switch (action) {
			...
			case R.id.accessibilityActionScrollDown: {
				final int viewportHeight = getHeight() - paddingBottom - paddingTop;
	...
	private View findFocusableViewInBounds(boolean topFocus, int top, int bottom) {
		if(true)
			return null; // FIXME
		List<View> focusables = null;//getFocusables(View.FOCUS_FORWARD);
	...
	public boolean fullScroll(int direction) {
		...
		if (down) {
			...
			if (count > 0) {
				View view = getChildAt(count - 1);
				mTempRect.bottom = view.getBottom() + paddingBottom;
		...
		if (nextFocused != null && isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
			...
			if (direction == View.FOCUS_UP && getScrollY() < scrollDelta) {
				...
				if (getChildCount() > 0) {
					...
					int screenBottom = getScrollY() + getHeight() - paddingBottom;
	...
	protected void measureChild(View child, int parentWidthMeasureSpec,
		...
		childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(Math.max(0, MeasureSpec.getSize(parentHeightMeasureSpec) - verticalPadding),
		                                                     MeasureSpec.UNSPECIFIED); /* was makeSafeMeasureSpec */
	...
	protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed,
		...
		final int childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec,
		                                                      ...
		                                                      + widthUsed,
		                                                      lp.width);
		final int usedTotal = paddingTop + paddingBottom + lp.topMargin + lp.bottomMargin + heightUsed;
		final int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(Math.max(0, MeasureSpec.getSize(parentHeightMeasureSpec) - usedTotal),
		                                                               MeasureSpec.UNSPECIFIED); /* was makeSafeMeasureSpec */
